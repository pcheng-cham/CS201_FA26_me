...
# names code chunk as "load-data"
#| label: load-data
# CHECK BACK FOR FIX
data(mtcars) # reads in the data & adds it to your environment as a dataframe
mtcars

mtcars <- mtcars |> # overwriting
  rownames_to_column("model")
View(mtcars) # displays table in new window
# head(mtcars, 10) # displays 10 rows
# summary(mtcars) #displays summary in console
# ?mtcars # opens help info

tail(mtcars) #how to view last few entries
```
...
# mean(mtcars) #WRONG
# mean(mpg) #WRONG
mean(mtcars $mpg) # $ lets you select a variable/volumn within df
```
...
# library(ggplot2) # REDUNDANT SINCE TIDyVERSE CONTAINS THIS LIBRARY ALREADY
