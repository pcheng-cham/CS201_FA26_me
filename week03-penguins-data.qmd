...
# considered as scraping, so in this case, was disabled by url provider
```
...
# changes label to "clumen_length_mm"
```
...
# autoformats; look up documentation for other options like Camel case, etc
```
...
summary(penguins_data)     # summmary
head(penguins_data)       # add ", 10" to see first 10 rows
tail(penguins_data, 5)    # sees 5 entries of tail end

mySkim <- skimr::skim(penguins_data)
```
...
?DataExplorer   # what does this do? ASK
...
# what do we add here? to get to histogram

ggplot(data = penguins_data, aes(x = flipper_length_mm)) +
  geom_histogram()

# warning is there for 2 entries being "weird"; CHECK LATER??
```
...
ggplot(data = penguins_data, aes(x = flipper_length_mm, fill = species)) +
  geom_histogram(alpha = 0.7) + # alpha changes transparency
  scale_fill_manual(values = c("darkorange", "darkorchid", "cyan4"), name = "Penguin Species") +
# order of colors matters; penguins are listed in alphabetical order, so this follows it
  #also changing name of legend
  theme_bw() + # changes background color
  labs(title = "Palmer Penguins' Flipper Length", # label changing
        subtitle = "Three species observed on the three islands in the Palmer Archipelago",
        x = "Flipper Length (mm)",
...
ggplot(data = penguins_data, aes(x = flipper_length_mm, fill = species)) +
  geom_histogram(alpha = 0.7) + # alpha changes transparency
  scale_fill_viridis_d(option = "turbo", name = "Penguin Species")
      # same as above code chunk but using preset color choice
  theme_bw() + # changes background color
  labs(title = "Palmer Penguins' Flipper Length", # label changing
       subtitle = "Three species observed on the three islands in the Palmer Archipelago",
       x = "Flipper Length (mm)",
...
ggplot(data = penguins_data, aes(y = flipper_length_mm, fill = species)) +
  geom_boxplot()
  # bolded line inbetween box is median
```
...
ggplot(data = penguins_data, aes(x = species, y = flipper_length_mm, fill = species)) +
  geom_boxplot() +
  # geom_point() + #needs x and y axis
  geom_jitter(size = 0.5) + # same as above but spread apart
  theme_bw() +
  ...
  labs(title = "Palmer Penguins' Flipper Length", # label changing
       ...
       x = "Flipper Length (mm)",
       y = "Number of Penguins") +
  #above is just copied/pasted from earlier code chunks
...
ggplot(data = penguins_data, aes(x = flipper_length_mm, y = body_mass_g, color = species, shape = island)) + #changed fill to color instead for scatterplot
                            #shape is for dot shape
  geom_point() +
...
myPlotName <- ggplot(data = penguins_data, aes(x = culmen_depth_mm, fill = species)) +
  geom_histogram()+
  theme_bw() +
  
  scale_fill_viridis_d(option = "mako", name = "Penguin Species") +
  labs(title = "Palmer Penguins' Culmen Length", # label changing
     subtitle = "Three species observed on the three islands in the Palmer Archipelago",
     x = "Flipper Length (mm)",
     y = "Number of Penguins")
...
myPlotName2 <- ggplot(data = penguins_data, aes(x = culmen_depth_mm, y = body_mass_g, color = species)) +
  geom_point() +
  theme_bw() +
  scale_color_viridis_d(option = "mako", name = "Penguin Species") +
  labs(title = "Palmer Penguins' Culmen Depth by Body Mass", # label changing
       subtitle = "Three species observed on the three islands in the Palmer Archipelago",
       x = "Culmen Depth (mm)",
       y = "Body Mass (g)")

ggplotly(myPlotName2)
```
